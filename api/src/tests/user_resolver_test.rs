...
use actors::user_actor::{create_user_actor_params::CreateUserActorParams, player_factory::PlayerFactory, user_actor_deps::UserActorDeps, user_actor_registry::UserActorRegistry};
...
use crate::{controllers::google_controller::GoogleUserInfo, user_and_actor_resolver::local_user_and_actor_resolver::LocalUserAndActorResolver};
...
pub async fn can_resolve_new_user()->Result<(),String> {
    ...
    let resolver=LocalUserAndActorResolver::new(Arc::clone(&user_provider),Arc::clone(&user_registry));
    let create_result=resolver.resolve_google_user_and_actor(&google_user_info, |domain_user|{
...
pub async fn can_resolve_existitng_user()->Result<(),String> {
    ...
    let lookup_result=resolver.resolve_google_user_and_actor(&google_user_info,|domain_user|{
        Ok(CreateUserActorParams{
            user_data:domain_user,
            user_actor_deps:user_actor_deps
        })}).await?;
...
pub async fn can_get_existing_user()->Result<(),String> {
    ...
    let lookup_result=resolver.resolve_existing_user_and_actor(&create_result.domain_user.id).await?;
    assert_eq!(lookup_result.domain_user.id,create_result.domain_user.id);
    Ok(())
}

fn create_user_deps()->UserActorDeps{
    UserActorDeps { player_factory: Arc::new(PlayerFactory{}), tracks_provider: Arc::new(LocalTrackStoreProvider::new()) }
}
