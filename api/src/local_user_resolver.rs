...
impl LocalUserResolver{
    ...
    pub fn new(user_provider:Arc<dyn UserProvider+Send+Sync>,user_registry:Arc<UserActorRegistry>)->LocalUserResolver{
        LocalUserResolver { user_provider: Arc::clone(&user_provider), user_registry: Arc::clone(&user_registry) }
    }
    
    pub async fn resolve_or_create_user<F>(&self,
        google_user_info:&GoogleUserInfo,
        build_actor_params:F)->Result<DomainUser,String>
        ...
        let actor=
            self.user_registry.get_or_spawn_user_actor(&user_id, actor_deps).await?;
        todo!()
    }
    ...
    pub async fn resolve_existing_user(&self,user_id:&str)->Result<ResolvedUser,String>{
        let maybe_actor=match self.user_provider.get_user_by_id(&user_id).await{
            Some(u)=>{
                match self.user_registry.get_actor(&u.id).await{
                    Some(actor)=>Ok(ResolvedUser{actor:actor,domain_user:u}),
