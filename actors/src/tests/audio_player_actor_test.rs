...
async fn test_can_write() -> Result<(), String> {
    ...
    let audio_player_actor = create_user_actor_with_track(create_meta(track.info),Arc::new(track.data), sink);
...
async fn test_can_pause() -> Result<(), String> {
    ...
    let sink = Box::new(TestSink {
     ...
     let actor_ref = create_user_actor_with_track(create_meta(track.info),Arc::new(track.data), sink);
...
fn create_meta(info:TrackInfo)->TrackMeta{
    let id=Ulid::new();
    TrackMeta{track_id:id.to_string(),track_info:info}
}
