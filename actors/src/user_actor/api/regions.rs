use domain::{actors::messages::regions::
...
impl Message<AddRegion> for UserActor{
    ...
    async fn handle(
        ...
        let set:RegionSet=self.region_set_provider.get_region_set(&msg.region_set_id).await?;
        let resolved_time=UserActor::resolve_end_time(&set, msg.start_time, msg.end_time_policy)?;

        let rez:RegionSet=
            self.region_set_provider.add_region(AddRegionParams{
                name:msg.name,
...
impl UserActor{
    pub fn resolve_end_time(
        ...
        let end_time=match end_time_policy{
                ...
                EndTimePolicy::FixedLength(len)=>{
                        Ok(new_region_start_time+len)
                }
            };
...
impl Message<EditRegion> for UserActor{
    ...
    async fn handle(
        ...
        let result:RegionSet=self.region_set_provider.edit_region(EditRegionParams{
            name:msg.name,
...
impl Message<DeleteRegion> for UserActor{
    ...
    async fn handle(
        ...
        let _=self.region_set_provider.delete_region(DeleteRegionParams{
            region_id:msg.region_id,
            region_set_id:msg.region_set_id
        }).await?;
