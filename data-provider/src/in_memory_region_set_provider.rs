...
impl RegionSetsProvider for InMemoryRegionSetProvider{
    ...
    async fn edit_region_set(&self, params:EditRegionSetParams)
        -> Result<RegionSet, String>{
            ...
            let set=match guard.get_mut(&params.region_set_id){
                    Some(set)=>set,
                    None=>return Err(format!("Could not find set with id {:?}",params.region_set_id))
            };
    ...
    async fn edit_region(&self, params: EditRegionParams)
        -> Result<RegionSet, String>{
            ...
            let region_opt = set.regions
                .iter_mut()
                .find(|r| r.region_id == params.region_id)
                .ok_or_else(||format!("Could not find region with id {:?}",params.region_id).to_string())?;
            if let Some(n)=params.name{
                region_opt.name=n;
            }
