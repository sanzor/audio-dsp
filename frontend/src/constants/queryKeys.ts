export const QUERY_KEYS = {
  ...
  transforms: {
    ...
    published: () => ['transforms', 'published'] as const,
  ...
  transformDrafts: {
    byId: (id: number) => ['transform-draft', id] as const,
    resolve: (ids: number[]) => ['transform-draft', 'resolve', ids.slice().sort((a, b) => a - b).join(',')] as const,
  },
  storedAudio: {
