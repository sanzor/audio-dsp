...
//     while this composite is playing back and this node is present in the
//     currently compiled playback graph (Phase 2) — never written into
...
export function NodeInspectorPanel({ nodeId, transformId, compositeTransformId, onClose }: NodeInspectorPanelProps) {
  ...
  const playbackGraph = useCreatorPlaybackStore((s) => s.playbackGraph);
  const nodeParamValues = useCreatorPlaybackStore((s) => s.nodeParamValues);
  const updateNodeParam = useCreatorPlaybackStore((s) => s.updateNodeParam);

  // "Is this node actually present in the compiled graph currently loaded
  // into the worklet" — not just "is this composite playing back at all",
  ...
  const isLive =
    ...
    playbackTransformId === compositeTransformId &&
    (playbackGraph?.executionOrder.some((n) => n.nodeId === nodeId) ?? false);
