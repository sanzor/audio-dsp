...
- The Creator composite canvas's preview compilation (`frontend/src/components/creator/composite/composite-preview-controls.ts`) directly imports and calls Editor's `frontend/src/audio/pipeline/GraphCompiler.ts` (`process`, `inputPortCountOf`, `inputPortIndexByName`) rather than maintaining an independent copy of graph-compilation logic. This is an accepted exception — pure, stateless compilation math, not Editor UI or mutable state — confirmed in `agents/decisions/0006-composite-feedback-cycle-support.md`. A future change to `GraphCompiler.ts`'s `CompiledNode`/`CompiledGraph`/`GraphInput` shapes or its cycle-handling behavior must be checked against this composite-preview call site too, not just Editor graph compilation.
