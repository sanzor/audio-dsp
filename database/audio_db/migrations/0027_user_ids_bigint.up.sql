-- Keep persisted user identity columns aligned with domain::domain_user::UserId.
-- UserId is an i64, so every users FK must be BIGINT rather than INTEGER.
BEGIN;
...
ALTER TABLE workspaces ALTER COLUMN created_by TYPE BIGINT;
ALTER TABLE workspace_members ALTER COLUMN user_id TYPE BIGINT;
ALTER TABLE subscriptions ALTER COLUMN user_id TYPE BIGINT;
ALTER TABLE purchased_products ALTER COLUMN user_id TYPE BIGINT;
ALTER TABLE invoices ALTER COLUMN user_id TYPE BIGINT;
ALTER TABLE usage ALTER COLUMN user_id TYPE BIGINT;
ALTER TABLE transform ALTER COLUMN owner_user_id TYPE BIGINT;
ALTER TABLE transform_grants ALTER COLUMN grantee_user_id TYPE BIGINT;
ALTER TABLE transform_grants ALTER COLUMN granted_by TYPE BIGINT;
