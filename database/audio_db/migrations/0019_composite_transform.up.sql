ALTER TABLE transform_draft ADD COLUMN graph_definition JSONB;

CREATE TABLE transform_composite (
  transform_id      BIGINT       PRIMARY KEY REFERENCES transform(transform_id) ON DELETE CASCADE,
...
-- column list changes (graph_definition is a new output column here) — drop first.
