ALTER TABLE transform
  ADD COLUMN kind TEXT NOT NULL DEFAULT 'primitive' CHECK (kind IN ('primitive', 'composite'));

ALTER TABLE transform ALTER COLUMN kind DROP DEFAULT;

-- Postgres disallows CREATE OR REPLACE FUNCTION when the RETURNS TABLE
-- column list changes (kind is a new output column here) — drop first.
