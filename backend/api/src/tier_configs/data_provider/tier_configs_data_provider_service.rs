...
impl TierConfigsDataProvider for TierConfigsDataProviderService {
    async fn get_tier_config(&self, tier: &Tier) -> Result<Option<DbTierConfig>, DataError> {
        sqlx::query_as::<_, DbTierConfig>(&format!("{SELECT} WHERE tier = $1"))
            ...
            .await
            .map_err(|e| { error!(error = %e, "get tier config failed"); DataError::from(e) })
    }
    ...
    async fn update_tier_config(&self, tier: &Tier, params: UpdateTierConfigParams) -> Result<Option<DbTierConfig>, DataError> {
        ...
        let max_tracks = params.max_tracks_per_project.unwrap_or(existing.max_tracks_per_project);
        let max_storage = params.max_storage_bytes.unwrap_or(existing.max_storage_bytes);
    ...
    async fn list_tier_configs(&self) -> Result<Vec<DbTierConfig>, DataError> {
        sqlx::query_as::<_, DbTierConfig>(SELECT)
            .fetch_all(&self.pool)
            .await
            .map_err(|e| { error!(error = %e, "list tier configs failed"); DataError::from(e) })
    }
}
