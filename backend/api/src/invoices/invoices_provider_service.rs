...
impl InvoicesProvider for InvoicesProviderService {
    ...
    async fn list_by_user_paginated(&self, user_id: &str, offset: i64, limit: i64) -> Result<(Vec<DbInvoice>, i64), ServiceError> {
        self.data_provider.list_by_user_paginated(user_id, offset, limit).await.map_err(ServiceError::from)
    }
    async fn list_all_paginated(&self, user_id: Option<&str>, offset: i64, limit: i64) -> Result<(Vec<DbInvoice>, i64), ServiceError> {
