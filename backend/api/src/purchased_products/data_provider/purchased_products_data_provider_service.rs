...
impl PurchasedProductsDataProvider for PurchasedProductsDataProviderService {
    async fn create_purchased_product(&self, params: CreatePurchasedProductParams) -> Result<DbPurchasedProduct, DataError> {
        ...
        .fetch_one(&self.pool)
        .await
        .map_err(|e| { error!(error = %e, "create purchased product failed"); DataError::from(e) })
    }
    ...
    async fn list_by_user(&self, user_id: domain::domain_user::UserId) -> Result<Vec<DbPurchasedProduct>, DataError> {
        sqlx::query_as::<_, DbPurchasedProduct>(&format!("{SELECT} WHERE user_id = $1 ORDER BY purchased_at DESC"))
            .bind(user_id)
            .fetch_all(&self.pool)
            .await
            .map_err(|e| { error!(error = %e, "list purchased products failed"); DataError::from(e) })
    }
    ...
    async fn delete_purchased_product(&self, id: PurchasedProductId) -> Result<bool, DataError> {
        sqlx::query("DELETE FROM purchased_products WHERE id = $1")
            ...
            .map_err(|e| { error!(id, error = %e, "delete purchased product failed"); DataError::from(e) })
    }
}
