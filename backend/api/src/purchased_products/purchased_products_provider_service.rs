...
impl PurchasedProductsProviderService {
    pub fn new(data_provider: Arc<dyn PurchasedProductsDataProvider>) -> Self { Self { data_provider } }
}
#[async_trait]
impl PurchasedProductsProvider for PurchasedProductsProviderService {
    async fn create_purchased_product(&self, params: CreatePurchasedProductParams) -> Result<DbPurchasedProduct, ServiceError> {
        self.data_provider.create_purchased_product(params).await.map_err(ServiceError::from)
    }
    ...
    async fn delete_purchased_product(&self, id: PurchasedProductId) -> Result<bool, ServiceError> {
        self.data_provider.delete_purchased_product(id).await.map_err(ServiceError::from)
    }
}
