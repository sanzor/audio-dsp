...
impl TransformsProvider for TransformsProviderService {
    ...
    async fn list_accessible_transform_ids(&self, user_id: UserId) -> Result<Vec<TransformId>, ServiceError> {
        self.data.list_accessible_transform_ids(user_id).await.map_err(ServiceError::from)
    }
