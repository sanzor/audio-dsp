...
const SELECT: &str =
    "SELECT id, user_id, tier, is_active, started_at::text, expires_at::text FROM subscriptions";
...
impl SubscriptionsDataProvider for SubscriptionsDataProviderService {
    ...
    async fn get_subscription(
        ...
        sqlx::query_as::<_, DbSubscription>(&format!("{SELECT} WHERE id = $1"))
            ...
            .await
            .map_err(|e| {
                error!(id, error = %e, "get subscription failed");
                DataError::from(e)
            })
    }
    ...
    async fn get_active_subscription_for_user(
        ...
        sqlx::query_as::<_, DbSubscription>(&format!(
            "{SELECT} WHERE user_id = $1 AND is_active = true ORDER BY started_at DESC LIMIT 1"
        ))
        .bind(user_id)
        .fetch_optional(&self.pool)
        .await
        .map_err(|e| {
            error!(error = %e, "get active subscription failed");
            DataError::from(e)
        })
    }
    ...
    async fn deactivate_subscription(&self, id: SubscriptionId) -> Result<bool, DataError> {
        sqlx::query("UPDATE subscriptions SET is_active = false WHERE id = $1")
            ...
            .map_err(|e| {
                error!(id, error = %e, "deactivate subscription failed");
                DataError::from(e)
            })
    }
    ...
    async fn list_by_user(
        &self,
        user_id: domain::domain_user::UserId,
    ) -> Result<Vec<DbSubscription>, DataError> {
        sqlx::query_as::<_, DbSubscription>(&format!(
            "{SELECT} WHERE user_id = $1 ORDER BY started_at DESC"
        ))
        .bind(user_id)
        .fetch_all(&self.pool)
        .await
