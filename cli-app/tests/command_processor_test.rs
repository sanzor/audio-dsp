use cli_app::{
    command::{Command, CommandResult}, command_dispatchers_provider::DispatchersProvider, command_processor::CommandProcessor, dispatchers::copy, processor::Processor, state::create_shared_state, track::TrackInfo
};
use rstest::rstest;
mod common;
...
pub fn can_run_upload_command() -> Result<(), String> {
    ...
    let mut processor = CommandProcessor::new(DispatchersProvider::new(), create_shared_state());
    load_command(&mut processor, name).unwrap();
    let upload_command = Command::Upload {
        name: Some(name.to_string()),
...
pub fn can_run_delete_command() -> Result<(), String> {
    ...
    let delete_command = Command::Delete {
        name: Some(name.to_string()),
    };
...
fn get_track_list(processor:&mut CommandProcessor)->Result<Vec<TrackInfo>,String>{
    serde_json::from_str(&processor.process_command(Command::Ls{})?.output).map_err(|e|e.to_string())
}
