...
impl CommandDispatch for CopyDispatcher {
    async fn dispatch(
        ...
        match envelope.command {
            Message::Copy { user_name,track_name: track_name, copy_name } => {
                self.internal_dispatch(user_name,track_name, copy_name, state).await
            }
...
impl CopyDispatcher {
    async fn internal_dispatch(
        ...
        let mut new_track = state
           .get_track_copy(&user_name, &track_name).await?;
